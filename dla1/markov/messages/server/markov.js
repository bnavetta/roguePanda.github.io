...
class MarkovGenerator {
  ...
  generate(seed) {
     ...
     if (seed) {
         var current = seed.substring(seed.length - this.n, seed.length);
         var output = seed;
     } else {
     ...
     if (!this.ngrams.hasOwnProperty(current)) {
         console.log("Skipping", current, "(", output, ")");
    ...
    if (seed && output.startsWith(seed)) {
        return output.substring(seed.length);
    }
